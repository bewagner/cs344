...
__device__ inline int calc_1D_pos(int x, int y, int numCols) {
	return y * numCols + x;
}

// Function to check wether given x and y values are within am image
__device__ inline bool is_in_image_boundaries(int x, int y, int numCols,
                                              int numRows) {
	return x > -1 && x < numCols && y > -1 && y < numRows;
}
...
__global__ void gaussian_blur(const unsigned char *const inputChannel,
	...
	if (!is_in_image_boundaries(thread_2D_pos.x, thread_2D_pos.y, numCols,
								numRows))
		return;

	// Accumulate the values from the old picture multiplied by the respective
	// filter value
	float new_value = 0;

	int k = 0;
	for (int i = -(filterWidth / 2); i < (filterWidth / 2) + 1; i++) {
		for (int j = -(filterWidth / 2); j < (filterWidth / 2) + 1; j++) {
			if (is_in_image_boundaries(thread_2D_pos.x + i, thread_2D_pos.y + j,
									   numCols, numRows)) {
				new_value += (float)inputChannel[calc_1D_pos(
						thread_2D_pos.x + i, thread_2D_pos.y + j, numCols)] *
					filter[k];
			} else {
				new_value += 127.0f * filter[k];
			}
			k++;
		}
	}

	outputChannel[thread_1D_pos] = static_cast<unsigned char>(new_value);
}
...
__global__ void separateChannels(const uchar4 *const inputImageRGBA,
	...
	uchar4 input_rgb_value = inputImageRGBA[thread_1D_pos];

	redChannel[thread_1D_pos] = input_rgb_value.x;
	greenChannel[thread_1D_pos] = input_rgb_value.y;
	blueChannel[thread_1D_pos] = input_rgb_value.z;
}
...
void allocateMemoryAndCopyToGPU(const size_t numRowsImage,
  ...
  checkCudaErrors(cudaMemcpy((void *)d_filter, (void *)h_filter, filter_bytes,
...
void your_gaussian_blur(const uchar4 *const h_inputImageRGBA,
  ...
  const dim3 gridSize((int)ceil((float)numCols / n_threads),
                      (int)ceil((float)numRows / n_threads));
  const dim3 blockSize(n_threads, n_threads);

  // Separate the three image channels
